...
#undef XX
    ...
    LogEvent::LogEvent(std::shared_ptr<Logger> logger, LogLevel::Level level, const char *file, int32_t line, uint32_t elapse,
                       uint32_t thread_id, uint32_t fiber_id, uint64_t time, const std::string &thread_name) : m_file(file),
                                                                                                               m_line(line),
                                                                                                               m_elapse(elapse),
    ...
    void Logger::setFormatter(const std::string &val)
    {
        ...
        if (new_val->isError())
        {
            std::cout << "Logger settFormatter name= " << m_name
